# 설계 01: CEP: Tizen 사용자 승인 프레임워크 — consent / consentd
...
v0.8은 번호가 있는 설계·가이드 디렉터리와 Tizen 형식 공개 C API 설명·예제, GBS에서
...
승인 기간·실행 전 검증의 책임을 정하며 실제 검증은 가이드 07에서 구분한다.
...
| U-21 | 설계와 가이드를 디렉터리로 구별하고 문서 번호·영문 README·Tizen 형식 C API 설명과 예제를 제공한다. |
| U-22 | .NET 승인 팝업을 GBS에서 빌드·TPK 패키징·설치하여 emulator PoC를 수행한다. |
| U-23 | argo 등 외부 연동은 별도 mock을 제공해 실제 API 경로에서 통합 시험하고 제품 연동과 구별한다. |
...
`05-decisions.ko.md`의 D-13을 따른다.
