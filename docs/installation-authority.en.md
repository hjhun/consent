...
The authority is `/opt/var/lib/consentd/installations.conf`, with root-only
ownership and a root-protected directory chain. The canonical `/opt/var` path is
...
`/opt/var/lib/consent-test`. The test daemon accepts this explicit inventory
