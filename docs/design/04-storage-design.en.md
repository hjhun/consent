...
## Receipt-preserving metadata compaction

The DB executor runs one atomic batch per 60 seconds, changing at most 128
candidate records. Invalid authorization payloads/edges, terminal prompt secrets
and CLOSED-session resume hashes can be cleared; revoked grants are removed
only when neither authorization nor artifact provenance references them.
Execution/request keys and fingerprints, artifact rows, recursive provenance,
cleanup ACKs, registry receipts and offline seeds remain. Identical invalid
execution retries stay STALE and changed retries stay CONFLICT; consumed ONCE
grants never become reusable. Failures roll back and use normal storage fencing.

This reclaims reusable SQLite space without VACUUM or deleting historical retry
rows. It does not implement general TTL pruning, wall-time bounds or lost-registry
reset. [Guide 09](../guides/09-storage-maintenance.en.md) gives the exact predicates,
indexes, test scope and separate unimplemented registry recovery design.
