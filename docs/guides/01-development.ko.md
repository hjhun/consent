# 가이드 01: consent 개발 가이드

이 문서는 저장소의 실제 구현을 설명합니다. 원본
[CEP](../design/01-consent-framework.md)는 설계 제안으로 보존합니다. 구현과 검증은
...
위 예시는 runtime과 test RPM을 설치합니다. `consent-devel`은 호환되는
`capi-base-common-devel` 의존성을 확보한 뒤 별도로 설치합니다. 기준 emulator의
0.4.82 runtime에는 일치하는 로컬 devel RPM이 없어 이전 설치 헤더·ABI 검증은
GBS SDK 설치 tree에서 수행했습니다. 의존성 검사를 우회하거나 헤더 설치만을 위해
공유 platform runtime을 업그레이드하지 않습니다. 선택적인 `consent-poc` RPM과
TPK 등록·설정은 가이드 08을 참고하세요.
