...
[구현 결정](05-decisions.ko.md)은 채택한 선택을 기록합니다.
[저장소 설계](04-storage-design.ko.md)와 [통신 규격](03-protocol.ko.md)은 각각의
...
[.NET UI PoC](../guides/08-consent-ui-poc.ko.md)는 명시적으로 격리된 배포의
승인 UI를 구현합니다. 전용 native worker가 `libconsent-poc.so.0`을 통해 별도
PID1 listener·state·authority·role 파일을 사용하며 엄격한 socket·역할 검증을
유지합니다. 실제 관측한 패키지별 SMACK 신원으로 같은 .NET launcher를 사용하는
다른 앱과 구별합니다. 나머지 참여자는 별도 native mock으로 시험하며 제품 서비스
연동과 구분합니다.
...
DB timer는 분당 한 차례 재시도 증거를 보존하는 metadata 정리도 수행합니다.
최대 128개 대상 record를 원자적으로 변경하며 request/authorization 신원과
artifact 정리 증거는 유지합니다. batch 변경량의 상한이고 전체 이력 크기나 scan
시간의 상한은 아닙니다. [저장소 관리](../guides/09-storage-maintenance.ko.md)를
참고하세요.
...
뜻하지는 않습니다. [설치 authority](../guides/03-installation-authority.ko.md)를 참고하세요.
