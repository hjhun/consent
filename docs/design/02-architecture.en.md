# Design 02: Architecture and trust boundaries

This guide describes the implementation in this repository. The CEP defines the
intended framework; [implementation decisions](05-decisions.en.md) record adopted
choices. [Storage design](04-storage-design.en.md) and [wire protocol](03-protocol.en.md)
...
The [.NET UI PoC](../guides/08-consent-ui-poc.en.md) implements the approval
participant for an explicitly isolated deployment. Its dedicated native worker
uses `libconsent-poc.so.0` and a separate PID1 listener, state, authority and
role file. The same strict socket and role checks remain active. Package-specific
observed SMACK identity distinguishes the UI from other apps sharing the .NET
launcher. Separate native mocks exercise the other participants; they are not
...
The DB timer also schedules one receipt-preserving metadata compaction batch
per minute. It changes at most 128 candidate records atomically and leaves
request/authorization identities and artifact cleanup evidence intact. This
limits batch mutations, not total history size or scan time; see
[storage maintenance](../guides/09-storage-maintenance.en.md).
...
See [installation authority](../guides/03-installation-authority.en.md).
