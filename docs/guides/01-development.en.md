# Guide 01: consent developer guide

This guide describes the implementation in this repository. The original
[CEP](../design/01-consent-framework.md) remains a design proposal. Implementation and
...
for package in consent consentd consent-tests; do
  sdb -s "$CONSENT_DEVICE" push "$CONSENT_RPM_DIR/$package-0.1.0-1.x86_64.rpm" /tmp/
done
sdb -s "$CONSENT_DEVICE" shell 'rpm -Uvh --replacepkgs --replacefiles /tmp/consent-0.1.0-1.x86_64.rpm /tmp/consentd-0.1.0-1.x86_64.rpm /tmp/consent-tests-0.1.0-1.x86_64.rpm'
...
The example installs runtime and test RPMs. Install `consent-devel` separately
when a matching `capi-base-common-devel` dependency is available. The reference
emulator's 0.4.82 runtime has no matching local devel RPM; prior installed-header
and ABI checks used the GBS SDK tree. Do not bypass RPM dependency checks or
upgrade a shared platform runtime solely to install these headers. The optional
`consent-poc` RPM and its TPK registration/setup are covered by Guide 08.
...
cleanup access only. See the [storage guide](../design/04-storage-design.en.md) for total DB
...
emulator C API scenarios; the [verification record](07-verification.en.md) identifies
...
[offline registration](04-offline-registration.en.md); ordinary clients never fall
